...
pub fn parse_config() -> ConfigOptions {
    let mut config:ConfigOptions = toml::from_str(&fs::read_to_string("./shttpd.conf")
    ...
    if config.options.default_page
        .as_ref()
    ...
    if config.options.page404.expect("failed to read config") && 
        !check_dir(append_dir(config.dir.html_dir
            .as_ref()
            .expect("invalid html directory")
            .to_string(), 
        "./404.html"
        ...
        eprintln!("404 page file does not exist: check page404 in config or create one in the html_dir");
        std::process::exit(1);
    }
    config
}
...
pub fn default_config() {
    fs::copy("./shttpd.conf", "shttpd.conf.old")
        .expect("failed to backup old config");

    fs::remove_file("./shttpd.conf")
        .expect("failed to delete current config file");
